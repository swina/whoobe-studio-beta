    ...
    <div v-if="slider">
        <div class="flex flex-col text-sm my-2 bg-gray-200 rounded p-1">
            <div class="flex flex-row my-1 p-1 border-b">
                <input type="number" min="0" max="100000" title="set 0 for autoplay" v-model="slider[context].delay"/>
            ...
            <div class="flex flex-row items-center justify-between my-1 p-1 border-b">
                ...
                <div class="text-sm" title="navigation icons color">
                    <moka-color attr="textcolor" v-model="slider[context].navigation.css" :css="slider[context].navigation.css"/>
                </div>
                
            </div>
            <div class="flex flex-col text-sm" title="navigation css">
                Navigation CSS
                <textarea v-model="slider[context].navigation.css"/>
            </div>
...
export default {
    ...
    mounted(){
        ...
        this.editor.current.hasOwnProperty('slider') ? this.context = 'slider' : this.context = 'gallery'
    }
}
</script>