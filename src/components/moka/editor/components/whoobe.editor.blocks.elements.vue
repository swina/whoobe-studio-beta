...
<div class="z-2xtop" v-if="schema && schema.keys">
        ...
        <div class="w-full cursor-pointer pb-20 border-l shadow overflow-y-auto bg-gray-800 text-gray-500">
            ...
            <template v-if="schema && schema.keys && !$attrs.importReusable"  v-for="(group,g) in schema.keys">
                ...
                <div v-if="currentGroup===group" class="w-full border-b border-gray-700 bg-gray-700 flex flex-row flex-wrap p-1 justify-center" :key="group + '_' + g">
                    <div v-for="(component,c) in schema[group]" class="flex flex-col p-1" :key="'component_' + c">
                        <div class="hover:bg-blue-200 hover:text-black flex-auto flex flex-col justify-center  p-1 text-center text-xs w-16 h-16 cursor-pointer shadow bg-gray-800"  v-if="!$attrs.newblock||component.tag==='container'" @click="setComponent(group,component,schema[group],c)">
                            <span v-if="group==='products'">{{ schema[group] }}</span>
                            <i class="material-icons text-blue-500">{{component.icon}}</i>
        ...
        <transition name="fade">
            ...
            <moka-modal
                ...
                <button class="sm w-12" @click="addElement(selected)">OK</button>
                </div>
            </moka-modal>
            <!-- </div> -->
        </transition>
